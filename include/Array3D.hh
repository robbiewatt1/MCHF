...
public:
	...
	friend Array3D<T> operator*(T constant, const Array3D<T> &array)
	{
		Array3D<T> newArray = Array3D<T>(array.m_nElementsX, array.m_nElementsY, array.m_nElementsZ);

		for (int i = 0; i < array.m_nElementsX; i++)
		{
			for (int j = 0; j < array.m_nElementsY; j++)
			{
				for (int k = 0; k < array.m_nElementsZ; k++)
				{
					newArray[i][j][k] = constant * array[i][j][k];
				}
			}
		}
		return newArray;
